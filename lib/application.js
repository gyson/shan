...
function Application(options) {
    ...
    this._toro_next = attachUseMethods(this, this)

    this._toro_registered_parse_fns = {}
    this._toro_registered_serve_fns = {}

    function parse(name) {
        let args = Array.prototype.slice.call(arguments)
        let handler = this.app._toro_registered_parse_fns[name]
        if (!handler) {
    ...
    function serve(name) {
        let args = Array.prototype.slice.call(arguments)
        let handler = this.app._toro_registered_serve_fns[name]
        if (!handler) {
...
Application.prototype.register = function (obj) {
    ...
    for (let plugin of arr) {
        if (typeof plugin.parse === 'function') {
            this._toro_registered_parse_fns[plugin.name] = _async(plugin.parse)
        }
        else if (typeof plugin.serve === 'function') {
            this._toro_registered_serve_fns[plugin.name] = _async(plugin.serve)
        }
...
Application.prototype.callback = function () {
    let app = this
    let next = this._toro_next(function noop() {})
...
module.exports.toro = Application
module.exports.default = Application

//
// Usage:
// import { toro, Promise, promisify, async } from 'toro'
// const { toro, Promise, promisify, async } = require('toro')
//
