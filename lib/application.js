    ...
    let asyncFn = fn.constructor.name === 'GeneratorFunction'
        ...
        : function () {
            try {
                ...
                // maybe a little bit safer use `instanceof`
...
function attachUseMethods(self, app) {
    ...
    self.use = function (fn) {
        ...
        // if it's generator, notice to useKoa
    ...
    return function (next) {
        return middlewareArray.reduceRight(function (next, curr, index) {
            if (Array.isArray(curr)) {
                // it's koa middleware (generators)
                function koaMiddleware(ctx) {
                    let i = curr.length
                    let gen = lazy(next, ctx)
                    while (i--) {
                        gen = curr[i].call(ctx, gen)
                    }
                    return co(gen)
                }
                koaMiddleware[I_WILL_RETURN_PROMISE] = true
